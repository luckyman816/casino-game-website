...
var fs = require("fs");
var serverConfig = JSON.parse(
    fs.readFileSync("../public/arcade_config.json", "utf8")
);
...
function CreatePlayerKA(ws, connections, playerId) {
    ...
    _self.modSystem = new rSystem.System(
        _self.emitter,
        sessionStorage,
        serverStorage,
        _self.modUtils,
    ...
    _self.ws.on("close", function cls() {
        ...
        for (let i = 0; i < players.length; i++) {
            if (
                players[i].modSystem == undefined ||
                players[i].modSystem.userId == -1
            ) {
                players.splice(i, 1);
            }
        }
    ...
    _self.ws.on("message", function incoming(message) {
        ...
        if (
            ...
            if (_self.MessageId == 0) {
                ...
                var response = _self.modUtils.EncodeMessage(
                    '...#{"code":200,"sys":{"heartbeat":30}}'
                );
        ...
        if (_self.modSystem.userId == -1) {
            if (
                incomingCookie["laravel_session"] == undefined ||
                incomingCookie["laravel_session"] == ""
            ) {
                ...
                let gameUrl =
                    ...
                    "/game/" +
                    incomingMess.gameName +
                    "/server?sessionId=" +
                    incomingMess.sessionId;
            ...
            _self.modSystem.gameName == undefined ||
            _self.modSystem.gameName == ""
        ) {
    ...
    _self.emitter.on("AuthAccept", async function () {
        ...
        */
        ...
        var gct = require("./arcade_server/games/" +
            _self.modSystem.gameName +
            ".js");
        _self.gameController = new gct.Game(
            _self.emitter,
            _self.modSystem,
            _self.modUtils
        );
    ...
    _self.emitter.on("Error", function (eMsg) {
        ...
        _self.emitter.emit(
            "outcomingMessage",
            '{"responseEvent":"error","responseType":"","serverResponse":"' +
            eMsg +
            '"}'
        );
    });
...
function CreatePlayer(ws, connections, playerId) {
    ...
    _self.ws.on("message", async function incoming(message) {
        ...
        //             incomingMess.sessionId;
        //         _self.modSystem.gameName = incomingMess.gameName;
        //         _self.modSystem.balanceUpdate(incomingMess.cookie, gameUrl, incomingMess.sessionId);
        //     }
        // }
        if (_self.modSystem.userId == -1) {
            if (
