...
serverConfig = JSON.parse(
    fs.readFileSync("../public/socket_config.json", "utf8")
);
        ...
        if (message.toString().split(":::")[1] != undefined) {
            ...
            if (param.vals != undefined) {
                if (param.irq != undefined) {
                    ws.send(
                        '~m~67~m~~j~{"err":0,"irs":1,"vals":[1,-2147483648,2,-503893983],"msg":null}'
                    );
...
server.listen(3000, () => {
    console.log("Server is listening on port 22054");
});
