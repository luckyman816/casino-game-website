                        ...
                        switch( $gameData['slotEvent'] ) 
                        {
                            ...
                            case 'A/u25':
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $DealCards = '';
                                    $slotSettings->SetGameData('TensOrBetterAMBet', $lastEvent->serverResponse->Bet);
                                    $slotSettings->SetGameData('TensOrBetterAMCurrentCards', $lastEvent->serverResponse->CurrentCards);
                                    $slotSettings->SetGameData('TensOrBetterAMCards', $lastEvent->serverResponse->Cards);
                                    $slotSettings->SetGameData('TensOrBetterAMbalanceFormated', $balanceFormated);
                                    ...
                                    {
                                        if( $slotSettings->Bet[$b] == $slotSettings->GetGameData('TensOrBetterAMBet') ) 
                                        {
                                            $betLevel = $b;
                                            break;
                                        }
                            ...
                            case 'A/u250':
                                ...
                                break;
                            case 'A/u253':
                                if( count($slotSettings->GetGameData('TensOrBetterAMCards')) <= 1 ) 
                                {
                                    exit();
                                }
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $resultCards0 = $slotSettings->GetGameData('TensOrBetterAMCurrentCards');
                                }
                                else
                                {
                                    $resultCards_ = $slotSettings->GetGameData('TensOrBetterAMCurrentCards');
                                    $cards = $slotSettings->GetGameData('TensOrBetterAMCards');
                                }
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    $totalWin = $cc[0] * $slotSettings->GetGameData('TensOrBetterAMBet');
                                    if( $totalWin <= $bank ) 
                                    {
                                        break;
                                    }
                                ...
                                $slotSettings->SetGameData('TensOrBetterAMTotalWin', $totalWin);
                                $DealCards = '';
                                for( $i = 0; $i < 5; $i++ ) 
                                {
                                    $DealCards .= ('2' . $resultCards[$i]->suit . dechex($resultCards[$i]->value));
                                ...
                                $slotSettings->SetGameData('TensOrBetterAMCollect', $response_collect);
                                $slotSettings->SetGameData($slotSettings->slotId . 'BankReserved', 0);
                                $logArr = (object)[
                                    'Bet' => $slotSettings->GetGameData('TensOrBetterAMBet'), 
                            ...
                            case 'A/u257':
                                $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                                $response_collect = $slotSettings->GetGameData('TensOrBetterAMCollect');
                                $balanceFormated = $slotSettings->HexFormat(round($slotSettings->GetBalance() * $floatBet));
                            ...
                            case 'A/u251':
                                ...
                                $slotSettings->SetGameData('TensOrBetterAMBet', $allbet);
                                $slotSettings->SetGameData('TensOrBetterAMCurrentCards', $resultCards);
                                $slotSettings->SetGameData('TensOrBetterAMCards', $cardsArr);
                                $slotSettings->SetGameData('TensOrBetterAMbalanceFormated', $balanceFormated);
                                break;
                            case 'A/u2514':
                                $balanceFormated = $slotSettings->GetGameData('TensOrBetterAMbalanceFormated');
                                $cardsID = [];
                                $cnt = 0;
                                for( $i = 2; $i <= 14; $i++ ) 
                                {
                                ...
                                $dCard = rand(2, 14);
                                $totalWin = $slotSettings->GetGameData('TensOrBetterAMTotalWin');
                                ...
                                $slotSettings->SetGameData('TensOrBetterAMDealerCard', $dCard);
                                $slotSettings->SetGameData('TensOrBetterAMDealerCardS', $dealerCard);
                                break;
                            case 'A/u2515':
                                $balanceFormated = $slotSettings->GetGameData('TensOrBetterAMbalanceFormated');
                                $cardsID = [];
                            ...
                            case 'A/u2513':
                                ...
                                $cWin = $slotSettings->GetGameData('TensOrBetterAMTotalWin');
                                $aWin = $cWin * 2;
                                ...
                                if( $isWin == 1 ) 
                                {
                                    $pCard = rand($slotSettings->GetGameData('TensOrBetterAMDealerCard'), 14);
                                }
                                else
                                {
                                    $pCard = rand(2, $slotSettings->GetGameData('TensOrBetterAMDealerCard'));
                                }
                                if( $slotSettings->GetGameData('TensOrBetterAMDealerCard') == $pCard ) 
                                {
                                    $totalWin = $cWin;
                                }
                                else if( $slotSettings->GetGameData('TensOrBetterAMDealerCard') < $pCard ) 
                                {
                                    $totalWin = $aWin;
                                }
                                else
                                ...
                                $response = '10d010' . $balanceFormated . $slotSettings->HexFormat(round($totalWin * 100)) . '10001a33e810' . $slotSettings->GetGameData('TensOrBetterAMDealerCardS') . $pCardsStr . '0000001723c1d23b21b10101023b21b020100000' . $pSelect . $slotSettings->GetGameData('TensOrBetterAMDealerCardS') . '#' . $pCard . '|' . $slotSettings->GetGameData('TensOrBetterAMDealerCardS');
                                if( $totalWin <= 0 ) 
                                {
                                    $totalWin = -1 * $cWin;
                                }
                                $response_log = '{"responseEvent":"gambleResult","serverResponse":{"totalWin":' . $totalWin . '}}';
                                $slotSettings->SaveLogReport($response_log, $slotSettings->GetGameData('TensOrBetterAMBet'), 1, $totalWin, 'double');
                                break;
                        }
