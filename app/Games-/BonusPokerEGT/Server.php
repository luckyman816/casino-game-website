...
>>>>>>> 7ca4ba0f61bb3bb1377303a455295c82f2bbc557
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"denominations":[' . implode(',', $denoms) . '],"bets":[[[' . $gameBets[0] . ',1,7000],[' . $gameBets[1] . ',1,14000],[' . $gameBets[2] . ',1,21000],[' . $gameBets[3] . ',1,42000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]]],"paytables":{"1":[1,2,3,4,5,8,25,40,80,50,250],"2":[1,2,3,4,5,8,25,40,80,50,800]},"jackpot":true,"jackpotMinBet":1,"jackpotMaxBet":100000000},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"26cb4a3e17bec5d3de3a93df0af8ef29","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":1599514248833}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData('BonusPokerEGTCards', $hist);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData($slotSettings->slotId . 'CurrentCards', $lastEvent->serverResponse->CurrentCards);
                            ...
                            case 'ping':
                                ...
                                break;
                            case 'draw':
                                $resultCards = $slotSettings->GetGameData('BonusPokerEGTCurrentCards');
                                $cardsArr = $slotSettings->GetGameData('BonusPokerEGTCards');
                                ...
                                array_shift($cardsArr);
                                $crdCount = 0;
                                $BankReserved = $slotSettings->GetGameData('BonusPokerEGTBankReserved');
                                $slotSettings->SetBank((isset($postData['slotEvent']) ? $postData['slotEvent'] : ''), $BankReserved);
                                $slotSettings->SetGameData('BonusPokerEGTBankReserved', 0);
                                $totalWin = 0;
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    $totalWin = $payrate * $slotSettings->GetGameData('BonusPokerEGTBet');
                                    if( $totalWin <= $bank ) 
                                    {
                                        break;
                                    }
                                ...
                                if( $totalWin > 0 ) 
                                {
                                    ...
                                    $state = 'gamble';
                                    $slotSettings->SetGameData('BonusPokerEGTGambleStep', 5);
                                }
                                $slotSettings->SetGameData('BonusPokerEGTTotalWin', $totalWin);
                                $DealCards = [];
                                ...
                                $winStepArr['pay_40'] = 7;
                                $winStepArr['pay_80'] = 8;
                                $winStepArr['pay_100'] = 9;
                                $winStepArr['pay_200'] = 10;
                                $winStepArr['pay_500'] = 11;
                                $winStep = $winStepArr['pay_' . $payrate];
                                $response = '{"responseEvent":"spin","responseType":"bet","serverResponse":{"CurrentCards":' . json_encode($slotSettings->GetGameData('BonusPokerEGTCurrentCards')) . ',"cardsArr":' . json_encode($cardsArr) . ',"state":"idle","slotLines":1,"slotBet":' . $slotSettings->GetGameData('BonusPokerEGTBet') . ',"totalFreeGames":' . $slotSettings->GetGameData('BonusPokerEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BonusPokerEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('BonusPokerEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"cards":[' . implode(',', $DealCards2) . ']}}';
                                ...
                                $slotSettings->SaveLogReport($response, $slotSettings->GetGameData('BonusPokerEGTBet'), 1, $totalWin, 'bet');
                                break;
                            case 'bet':
                                $betline = $postData['bet']['bet'] / 100;
                                $allbet = $betline;
                                ...
                                if( $postData['slotEvent'] != 'freespin' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData('BonusPokerEGTCurrentFreeGame', 0);
                                    $slotSettings->SetGameData('BonusPokerEGTTotalWin', 0);
                                    $slotSettings->SetGameData('BonusPokerEGTFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                    $bonusMpl = 1;
                                }
                                else
                                {
                                    $slotSettings->SetGameData('BonusPokerEGTCurrentFreeGame', $slotSettings->GetGameData('BonusPokerEGTCurrentFreeGame') + 1);
                                ...
                                $slotSettings->SetGameData('BonusPokerEGTGambleStep', 5);
                                $hist = $slotSettings->GetGameData('BonusPokerEGTCards');
                                $cardsArr = [];
                                $sqNum = 0;
                                $crdID = [
                                    14, 
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    if( $totalWin <= $bank ) 
                                    {
                                        $slotSettings->SetGameData('BonusPokerEGTBankReserved', $totalWin);
                                        break;
                                    }
                                }
                                $slotSettings->SetGameData('BonusPokerEGTBet', $allbet);
                                $slotSettings->SetGameData('BonusPokerEGTCurrentCards', $resultCards);
                                $slotSettings->SetGameData('BonusPokerEGTCards', $cardsArr);
                                $DealCards = [];
                                for( $i = 0; $i < 5; $i++ ) 
                                {
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData('BonusPokerEGTJackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"CurrentCards":' . json_encode($slotSettings->GetGameData('BonusPokerEGTCurrentCards')) . ',"cardsArr":' . json_encode($cardsArr) . ',"state":"' . $state . '","slotLines":1,"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('BonusPokerEGTFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BonusPokerEGTCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('BonusPokerEGTBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"cards":[' . implode(',', $DealCards) . ']}}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData('BonusPokerEGTTotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData('BonusPokerEGTTotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData('BonusPokerEGTJackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData('BonusPokerEGTJackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData('BonusPokerEGTJackSteps', $jackStep);
                                break;
                            case 'gamble':
                                $Balance = $slotSettings->GetBalance();
                                $isGambleWin = rand(1, $slotSettings->GetGambleSettings());
                                $dealerCard = '';
                                $totalWin = $slotSettings->GetGameData('BonusPokerEGTTotalWin');
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData('BonusPokerEGTGambleStep', $slotSettings->GetGameData('BonusPokerEGTGambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData('BonusPokerEGTGambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData('BonusPokerEGTBonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambleAttemptsLeft":' . $slotSettings->GetGameData('BonusPokerEGTGambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
