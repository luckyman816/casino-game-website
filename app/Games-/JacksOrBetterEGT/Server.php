...
>>>>>>> 7ca4ba0f61bb3bb1377303a455295c82f2bbc557
                        ...
                        switch( $aid ) 
                        {
                            ...
                            case 'settings':
                                ...
                                $result_tmp[] = '{"complex":{"balance":' . $balanceInCents . ',"balanceRaw":' . round($balanceInCents * $slotSettings->CurrentDenom) . ',"denominations":[' . implode(',', $denoms) . '],"bets":[[[' . $gameBets[0] . ',1,7000],[' . $gameBets[1] . ',1,14000],[' . $gameBets[2] . ',1,21000],[' . $gameBets[3] . ',1,42000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]],[[' . $gameBets[0] . ',1,14000],[' . $gameBets[1] . ',1,28000],[' . $gameBets[2] . ',1,42000],[' . $gameBets[3] . ',1,56000],[' . $gameBets[4] . ',2,70000]]],"paytables":{"1":[1,2,3,4,5,9,25,50,250],"2":[1,2,3,4,5,9,25,50,800]},"jackpot":true,"jackpotMinBet":1,"jackpotMaxBet":100000},"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":-1,"sessionKey":"f470855a38998b4c1678a0fd7d0ea815","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameResponse","command":"settings","eventTimestamp":1567091239811}';
                                break;
                            case 'subscribe':
                                $hist = [
                                    78, 
                                ...
                                shuffle($hist);
                                $slotSettings->SetGameData($slotSettings->slotId . 'Cards', $hist);
                                ...
                                if( $lastEvent != 'NULL' ) 
                                {
                                    ...
                                    $slotSettings->SetGameData($slotSettings->slotId . 'InternalState', $lastEvent->serverResponse->InternalState);
                            ...
                            case 'ping':
                                ...
                                break;
                            case 'draw':
                                if( $slotSettings->GetGameData($slotSettings->slotId . 'InternalState') != 'deal' ) 
                                {
                                    $response = '{"responseEvent":"error","responseType":"","serverResponse":"invalid state"}';
                                    exit( $response );
                                }
                                $resultCards = $slotSettings->GetGameData($slotSettings->slotId . 'CurrentCards');
                                ...
                                for( $i = 0; $i <= 2000; $i++ ) 
                                {
                                    ...
                                    $totalWin = $payrate * $slotSettings->GetGameData($slotSettings->slotId . 'Bet');
                                    if( $totalWin <= $bank ) 
                                    {
                                        break;
                                    }
                                ...
                                $winStepArr['pay_25'] = 6;
                                $winStepArr['pay_50'] = 7;
                                $winStepArr['pay_250'] = 8;
                                $winStep = $winStepArr['pay_' . $payrate];
                                $response = '{"responseEvent":"spin","responseType":"bet","serverResponse":{"InternalState":"' . $slotSettings->GetGameData($slotSettings->slotId . 'InternalState') . '","cardsArr":' . json_encode($cardsArr) . ',"state":"idle","slotLines":1,"slotBet":' . $slotSettings->GetGameData($slotSettings->slotId . 'Bet') . ',"totalFreeGames":' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"cards":[' . implode(',', $DealCards2) . ']}}';
                                ...
                                $slotSettings->SaveLogReport($response, $slotSettings->GetGameData($slotSettings->slotId . 'Bet'), 1, $totalWin, 'bet');
                                $slotSettings->SetGameData($slotSettings->slotId . 'InternalState', 'draw');
                                break;
                            case 'bet':
                                $betline = $postData['bet']['bet'] / 100;
                                $allbet = $betline;
                                ...
                                $slotSettings->SetGameData($slotSettings->slotId . 'InternalState', 'deal');
                                $slotSettings->SetGameData($slotSettings->slotId . 'Bet', $allbet);
                                $slotSettings->SetGameData($slotSettings->slotId . 'CurrentCards', $resultCards);
                                ...
                                if( $jackRandom == 1 ) 
                                {
                                    $state = 'jackpot';
                                    $isJack = 'true';
                                    $slotSettings->SetGameData($slotSettings->slotId . 'JackSteps', [
                                        0, 
                                        0, 
                                        0, 
                                        0
                                ...
                                $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"InternalState":"' . $slotSettings->GetGameData($slotSettings->slotId . 'InternalState') . '","cardsArr":' . json_encode($cardsArr) . ',"state":"' . $state . '","slotLines":1,"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData($slotSettings->slotId . 'FreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData($slotSettings->slotId . 'CurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData($slotSettings->slotId . 'BonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"cards":[' . implode(',', $DealCards) . ']}}';
                                ...
                                break;
                            case 'collect':
                                $result_tmp[] = '{"complex":{"gameCommand":"collect"},"state":"idle","winAmount":' . ($slotSettings->GetGameData($slotSettings->slotId . 'TotalWin') * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","balance":' . $balanceInCents . ',"sessionKey":"41be9e65e0ff03a65e8c93576bf61130","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                $slotSettings->SetGameData($slotSettings->slotId . 'TotalWin', 0);
                                break;
                            case 'jackpot':
                                $jackStep = $slotSettings->GetGameData($slotSettings->slotId . 'JackSteps');
                                $jackWinID = $slotSettings->GetGameData($slotSettings->slotId . 'JackWinID');
                                if( $jackStep[0] >= 3 || $jackStep[1] >= 3 || $jackStep[2] >= 3 || $jackStep[3] >= 3 || !$slotSettings->HasGameData($slotSettings->slotId . 'JackSteps') ) 
                                {
                                    exit();
                                }
                                $winAmount = 0;
                                ...
                                $slotSettings->SetGameData($slotSettings->slotId . 'JackSteps', $jackStep);
                                break;
                            case 'gamble':
                                ...
                                $gambleWin = 0;
                                $statBet = $totalWin;
                                $slotSettings->SetGameData($slotSettings->slotId . 'GambleStep', $slotSettings->GetGameData($slotSettings->slotId . 'GambleStep') - 1);
                                ...
                                else
                                {
                                    ...
                                    $totalWin = 0;
                                    $slotSettings->SetGameData($slotSettings->slotId . 'GambleStep', 0);
                                    if( $postData['bet']['color'] == '1' ) 
                                    {
                                        $tmpCards = [
                                            '1', 
                                ...
                                $gtype = 1;
                                $gtype0 = 1;
                                if( $slotSettings->GetGameData($slotSettings->slotId . 'BonusStart') == 1 ) 
                                {
                                    $gtype0 = 2;
                                }
                                $result_tmp[] = '{"complex":{"gambleAttemptsLeft":' . $slotSettings->GetGameData($slotSettings->slotId . 'GambleStep') . ',"card":' . $dealerCard . ',"jackpot":false,"gameCommand":"gamble"},"state":"' . $gambleState . '","winAmount":' . ($totalWin * 100) . ',"gameIdentificationNumber":' . $postData['gameIdentificationNumber'] . ',"gameNumber":"","sessionKey":"88e2b82e6537db4a339e4e1b5ce462cd","msg":"success","messageId":"' . $postData['messageId'] . '","qName":"app.services.messages.response.GameEventResponse","command":"bet","eventTimestamp":' . time() . '}';
                                break;
                        }
