...
namespace VanguardLTE\Http\Controllers\Web\Frontend\Auth
{
    ...
    class AuthController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function API(\VanguardLTE\Http\Requests\Auth\RegisterRequest $request)
        {
        ...
        // For Game API
        public function postAPIRegister(\Illuminate\Http\Request $request)
        {
            $id = $request->id;
            $email = $request->email;
            $name = $request->name;
            ...
            $provideAPI = $request->header("api");
		    if($provideAPI == ""){
                echo '{"responseEvent":"error","responseType":"start","serverResponse":"API not Authorized"}';
                return ;
                exit();
            }

            if(!$id || !$email || !$name || !$cburl || !$ucurl || !$balanceId || !$currency || !$provideAPI){
                echo '{"responseEvent":"error","responseType":"start","serverResponse":"You must enter all fields"}';
                return ;
                exit();
            }

            $Operator = new \VanguardLTE\Operator;
            $Operator->opid = $id;
            $Operator->ucurl = $ucurl;
            $Operator->cburl = $cburl;
            $Operator->save();

            $operator = \VanguardLTE\Operator::where(['opid'=>$id])->get()->toarray();
            ...
            $User->shop_id = 1;
            $User->balance = 0;
            $User->balance_id = $balanceId;
            $User->opid = $operator[0]['id']."_".$operator[0]['opid'];
