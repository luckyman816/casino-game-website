    ...
    class GamesController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function go(\Illuminate\Http\Request $request, $game, $prego = '')
        {
           ...
           if( $detect->isMobile() || $detect->isTablet() ) 
            ...
            $is_api = false;
            return view('frontend.games.list.' . $game->name, compact('slot', 'game', 'is_api', 'device'));
        }
        ...
        public function server(\Illuminate\Http\Request $request, $game)
        {
            ...
            echo json_decode($server->get($request, $game, auth()->user()->id));
            // send data to node js
            // getBalance
            $userData = \VanguardLTE\User::where('opid', auth()->user()->opid)->where('id', auth()->user()->id)->get()->toarray();

            $operator = \VanguardLTE\Operator::where(['opid' => explode('_', auth()->user()->opid)[1]])->get()->toarray();
            $headers = [
                'authorization' => $operator[0]['opid']
            ];
            Http::withHeaders($headers)->post($operator[0]['cburl'], [
                'data' =>  $userData,
            ]);
            // $response = json_decode($response->header('data'));




            // $ch = curl_init();
            // curl_setopt($ch, CURLOPT_URL, 'http://192.168.114.61/casinoAPI/updateBalance');

            // curl_setopt($ch, CURLOPT_HEADER, 0);
            // curl_setopt($ch, CURLOPT_HTTPHEADER, array('Expect:'));
            // curl_setopt($ch, CURLOPT_PORT, 2002);
        ...
        // For Game API
        public function goAPI(\Illuminate\Http\Request $request)
        {
            $gameID = $request->query('gameID');
            $opid = $request->query('opid');
            $requestAPI = $request->query('api');
            // Check API
            $api_status = \VanguardLTE\Api::where(['keygen' => $requestAPI])->first();
            if (!$api_status->keygen || !$requestAPI) {
                return response()->json([trans('Your api is not registered!!!')], 500);
            }
            // End
            if (!$gameID || !$opid) {
                return $this->errorHandler("User ID or Token is null");
            };

            $operator = \VanguardLTE\Operator::where(['opid' => $opid])->get()->toarray();
            $headers = [
                'authorization' => $operator[0]['opid']
            ];
            $response = Http::withHeaders($headers)->post($operator[0]['ucurl'], [
                'opid' =>  $operator[0]['opid'],
            ]);
            $response = json_decode($response->header('data'));

            if (!$response->userId) {
                echo '{"responseEvent":"error","responseType":"error","serverResponse":"Your server must send information"}';
                exit();
            }
            ...
            $cburl = $operator[0]['cburl'];
            $id =  $operator[0]['id'] . "_" . $userId;
            $user = '';
            if (\VanguardLTE\User::where('opid', '=', $id)->first()) {
                $user = \VanguardLTE\User::where('opid', '=', $id)->first();
                \VanguardLTE\User::where('opid', '=', $id)->update(array(
                    'username' => $username,
                    'balance' => $balance,
                ));
            } else {
            ...
            if (!count($gameInfo->toArray())) {
                return $this->errorHandler("Game doesn't exsits");
            };
            ...
            // $cburl = $operator->cburl;
            $slot = new $object($game->name, $userId);

            return view('frontend.games.list.' . $game->name, compact('slot', 'game', 'is_api', 'userId', 'cburl', 'balanceId'));
            // } else {
            //     return $this->errorHandler("Invalid Operator Id.");
            // }
        }
        ...
        public function getGameList(\Illuminate\Http\Request $request)
        {
            ...
            if(!$apiData) {
                return $this->errorHandler("Invalid API Key");
            }
            ...
            $games = $games->whereIn('device', $devices);
            ...
            if ($category1 != '') {
                ...
                $ids = \VanguardLTE\GameCategory::whereIn('category_id', $categories)->groupBy('game_id')->pluck('game_id')->toArray();
                if (count($ids) > 0) {
                    $games = $games->whereIn('original_id', $ids);
                } else {
                    $games = $games->where('id', 0);
                }
            }
            ...
            $game_ids = $games->groupBy('id')->pluck('id')->toArray();

            $games =  \VanguardLTE\Game::join("game_categories", "games.original_id", "=", "game_categories.game_id")

                ->join("categories", "categories.id", "=", "game_categories.category_id")

                ->where(['games.view' => 1, 'games.shop_id' => $apiData->shop_id])

                ->whereIn('games.device', $devices)

                ->whereIn('games.id', $game_ids)

                ->groupBy('games.id')

                ->get(["games.id", "games.name", "games.title", "game_categories.game_id", "game_categories.category_id", "categories.position", "categories.href"]);
            ...
            foreach ($games as $key => $gameItem) {
                ...
                $newGameItems['id'] = $gameItem['id'];

                $newGameItems['name'] = $gameItem['name'];

                $newGameItems['title'] = $gameItem['title'];

                $newGameItems['category_id'] = $gameItem['category_id'];

                $newGameItems['category_name'] = $gameItem['href'];

                $newGameItems['image_url'] = request()->root().'/frontend/Default/ico/' . $gameItem['name'] . '.jpg';

                $newGames[$key] = $newGameItems;
            }

            return $newGames;
        }

        public function getGameList_old(\Illuminate\Http\Request $request)
        {
            $category1 = $request->category;
            $isMobile = $request->isMobile ? true : false;
            $category2 = '';
            ...
            $games =  \VanguardLTE\Game::join("game_categories", "games.id", "=", "game_categories.game_id")

                ->join("categories", "categories.id", "=", "game_categories.category_id")

                ->where(['games.view' => 1, 'games.shop_id' => 0])

                ->whereIn('games.device', [1, 2])
            ...
            foreach ($games as $key => $gameItem) {
                ...
                $newGameItems['image_url'] = '/frontend/Default/ico/' . $gameItem['name'] . '.jpg';
        ...
        public function getGameCategory(\Illuminate\Http\Request $request)
        {
            $api = $request->header("api");
            $apiData = \VanguardLTE\API::where(['keygen' => $api])->first();
            if(!$apiData) {
            ...
            $category = array([
                'name' => \VanguardLTE\Category::get()->toarray(),
                'match' => \VanguardLTE\GameCategory::get()->toarray()
            ]);
            $categories = \VanguardLTE\Category::where([
                'parent' => 0,
                'shop_id' => 0
            ...
            $category = array([
                ...
                'name' => \VanguardLTE\Category::where(['parent' => 0,'shop_id' => 0])->get()->toArray(),
