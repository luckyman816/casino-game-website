    ...
    class DashboardController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function game_stat(\Illuminate\Http\Request $request)
        {
            ...
            if($request->api != '')
            {
                ...
                $statistics = $statistics->where('users.api', '=',$request->api);
            }
            if ($request->user != '')
            {
                $statistics = $statistics->join('users', 'users.id', '=', 'stat_game.user_id');
            ...
            $betSum = \VanguardLTE\StatGame::whereIn('id', $statistics)->sum('bet');
            $winSum = \VanguardLTE\StatGame::whereIn('id', $statistics)->sum('win');
            $profitSum = $betSum-$winSum;
            return view('backend.stat.game_stat', compact('game_stat', 'count', 'page', 'perPage','betSum','winSum','profitSum'));
        }
