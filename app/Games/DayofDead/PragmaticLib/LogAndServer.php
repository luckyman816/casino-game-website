...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        $trailkey = array_keys($toServer, 'trail='.$toLog['trail'])[0];
        var_dump('5_1_1');
        
        // add extra keys
        $addKeys = ['rs_t', 'rs', 'rs_p', 'rs_c', 'rs_m', 'rs_win', 'ep', 'is', 'trail', 'gwm', 'wmv', 'wmt', 'puri'];
        foreach($addKeys as $val){
            if(array_key_exists($val, $slotArea)){
                if($val == 'rs_win')
                    $slotArea[$val] += $win['TotalWin'];
                $toLog[$val] = $slotArea[$val];
                switch($val){ 
                    case 'trail':
                        $toServer[$trailkey] = 'trail='.$toLog[$val];
                        break;
                    case 'is':
                        $toServer[] = $val.'='.implode(',', $slotArea[$val]);
                        break;
                    case 'gwm':
                        $toServer[] = $val.'='.$slotArea[$val];
                        $toLog['tw'] += $log['tw'];
        ...
        if($pur === '0'){
            ...
            $toLog['trail'] = 'mode~base;markers~fs_trig';
            $toServer[$trailkey] = 'trail='.$toLog['trail'];
            var_dump('5_1_1_2.8');
