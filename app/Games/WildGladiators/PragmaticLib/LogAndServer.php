...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        if(array_key_exists('ep', $slotArea)){
            ...
            $toServer[] = 'is='.implode(',', $slotArea['is']);
        }
        if(array_key_exists('rwd', $slotArea)){
            $toServer[] = 'is='.implode(',', $slotArea['is']);
            $toServer[] = 'aw=10';
            $toServer[] = 'rwd='.$slotArea['rwd'];
        }
        else {
            $toServer[] = 'gwm='.$slotArea['gwm'];
            $toServer[] = 'aw='.($slotArea['gwm'] - 1);
        }
        var_dump('5_1_0');
        ...
        if($pur === '0'){
            $t_slotarea = $slotArea['SlotArea'];
            if(array_key_exists('is', $slotArea))
                $t_slotarea = $slotArea['is'];
            $psym = SlotArea::getPsym($gameSettings, $t_slotarea, $bet, $lines, $slotArea['gwm']);
            ...
            $scatterCnt = count(array_keys($t_slotarea, 1));
            ...
            $toLog['w'] += $psym['psymwin'];
            $toLog['tw'] += $psym['psymwin'];
