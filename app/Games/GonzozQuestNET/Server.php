<?php 
namespace VanguardLTE\Games\GonzozQuestNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('GonzozQuestNETBonusWin', 0);
                            $slotSettings->SetGameData('GonzozQuestNETFreeGames', 0);
                            $slotSettings->SetGameData('GonzozQuestNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('GonzozQuestNETTotalWin', 0);
                            $slotSettings->SetGameData('GonzozQuestNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('GonzozQuestNETCurrentFreeGame') < $slotSettings->GetGameData('GonzozQuestNETFreeGames') && $slotSettings->GetGameData('GonzozQuestNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('GonzozQuestNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('GonzozQuestNETTotalWin', 0);
                                $slotSettings->SetGameData('GonzozQuestNETBet', $betline);
                                $slotSettings->SetGameData('GonzozQuestNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('GonzozQuestNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('GonzozQuestNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('GonzozQuestNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('GonzozQuestNETCurrentFreeGame', $slotSettings->GetGameData('GonzozQuestNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('GonzozQuestNETBonusWin', $slotSettings->GetGameData('GonzozQuestNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('GonzozQuestNETTotalWin', $slotSettings->GetGameData('GonzozQuestNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('GonzozQuestNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('GonzozQuestNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('GonzozQuestNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('GonzozQuestNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('GonzozQuestNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('GonzozQuestNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('GonzozQuestNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('GonzozQuestNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('GonzozQuestNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('GonzozQuestNETFreeGames');
                                $fsl = $slotSettings->GetGameData('GonzozQuestNETFreeGames') - $slotSettings->GetGameData('GonzozQuestNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('GonzozQuestNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('GonzozQuestNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('GonzozQuestNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('GonzozQuestNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
