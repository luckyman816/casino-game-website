...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if ($command == 'commands') {
                ...
                if (is_array($gameData) && array_key_exists('gameActionId', $gameData) && $gameData['gameActionId'] == 'INIT') {
                    if (!$log) $log = ["Reels" => $settings->randomSlotArea, 'Winnings' => [], 'TotalWin' => 0, 'Bet' => 10, 'Lines' => 10];
                ...
                switch ($command) {
                    case 'PLAY':
                        ...
                        list($winnings, $readyReels, $slotArea, $totalWin, $params) = Spin::get($bet, $lines, $settings, $bank, $shop->percent, $game->stat_in, $game->stat_out);
                        ...
                        $toLog = ['Winnings' => $winnings, 'Reels' => $readyReels, 'SlotArea' => $slotArea, 'TotalWin' => $totalWin, 'Bet' => $bet, 'Lines' => $lines, 'Params' => $params];

                        // вернуть ответ с сервера
                        $response = Play::get((int)round($user->balance * 100), $winnings, $readyReels, $bet, $totalWin, $params);
                    ...
                    case 'FINISH_GAME':
                        ...
                        $user->save();
                        $response = Play::get((int)round($user->balance * 100), $log['Winnings'], $log['Reels'], $log['Bet'], $log['TotalWin'], $log['Params'],  true);
                        $log['Winnings'] = [];
                        //$log['Bet'] = 0;
                        $log['TotalWin'] = 0;
