...
namespace VanguardLTE\Games\CrownofFire\PragmaticLib;
...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        $addToServer = [];
        var_dump('5_1_1');
        $pysmArea = array_key_exists('SymbolsAfterExpanding', $slotArea) ? $slotArea['SymbolsAfterExpanding'] : $slotArea['SlotArea'];
        $psym = SlotArea::getPsym($gameSettings, $pysmArea, $bet, $lines);
        if($psym['psymwin']){
            $addToLog = [
                'psym' => $psym['psym']
            ];
