<?php 
namespace VanguardLTE\Games\TwinSpinNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('TwinSpinNETBonusWin', 0);
                            $slotSettings->SetGameData('TwinSpinNETFreeGames', 0);
                            $slotSettings->SetGameData('TwinSpinNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('TwinSpinNETTotalWin', 0);
                            $slotSettings->SetGameData('TwinSpinNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('TwinSpinNETCurrentFreeGame') < $slotSettings->GetGameData('TwinSpinNETFreeGames') && $slotSettings->GetGameData('TwinSpinNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('TwinSpinNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('TwinSpinNETTotalWin', 0);
                                $slotSettings->SetGameData('TwinSpinNETBet', $betline);
                                $slotSettings->SetGameData('TwinSpinNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('TwinSpinNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('TwinSpinNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('TwinSpinNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('TwinSpinNETCurrentFreeGame', $slotSettings->GetGameData('TwinSpinNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('TwinSpinNETBonusWin', $slotSettings->GetGameData('TwinSpinNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('TwinSpinNETTotalWin', $slotSettings->GetGameData('TwinSpinNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('TwinSpinNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('TwinSpinNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('TwinSpinNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('TwinSpinNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('TwinSpinNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('TwinSpinNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('TwinSpinNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('TwinSpinNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('TwinSpinNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('TwinSpinNETFreeGames');
                                $fsl = $slotSettings->GetGameData('TwinSpinNETFreeGames') - $slotSettings->GetGameData('TwinSpinNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('TwinSpinNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('TwinSpinNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('TwinSpinNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('TwinSpinNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
