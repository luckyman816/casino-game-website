...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        $wmvkey = array_keys($toServer, 'wmv='.$toLog['wmv'])[0];
        var_dump('5_1_1');

        // add keys related to extra wild and multi on scatter
        $addKeys = ['srf', 'is', 'gwm', 'rmul'];
        foreach($addKeys as $val){
            ...
            if(array_key_exists($val, $slotArea)){
                ...
                switch($val){ 
                    case 'srf':
                        $toServer[] = 'srf='.$toLog[$val];
                        break;
                    case 'is':
                        $toServer[] = 'is='.implode(',', $toLog[$val]);
                        break;
                    case 'gwm':
                        $toServer[]='gwm='.$toLog[$val];
                        $toServer[$wmvkey] = 'wmv='.$toLog[$val];
                        break;
                    case 'rmul':
                        $toServer[] = 'rmul='.$toLog[$val];
                        break;
                    default:
