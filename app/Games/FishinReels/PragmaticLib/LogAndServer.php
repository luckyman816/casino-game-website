...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        // add extra keys
        $addKeys = ['mo', 'mo_t', 'accv', 'accm', 'fsmore', 'ra2_awd_id', 'ra2_avl_awd', 'trail', 'dsa', 'dsam', 'ds'];
        ...
        if($log && array_key_exists('fs', $log)){
            if($log['fs'] == $log['fsmax'] && !array_key_exists('fsmore', $slotArea)){
                $addToLog = [
                    'fs_total' => $log['fs'],
                ...
                $fsmore = 0;
                if(array_key_exists('fsmore', $slotArea))
                    $fsmore = $slotArea['fsmore'];
                $addToLog = [
                    'fsmul' => $log['fsmul'],
                ...
                if($pur == '1'){
                    $addToLog['fsmore'] = $gameSettings['settings_addfs'];
