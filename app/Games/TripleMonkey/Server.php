...
$linesId = [];
                            ...
                            $new_rtp_user = \VanguardLTE\User::lockForUpdate()->find($userId);
                                $new_rtp_shop = \VanguardLTE\Shop::find($new_rtp_user->shop_id);
                                $new_rtp_Game = \VanguardLTE\Game::where([
                                    'name' => $game, 
                                    'shop_id' => $new_rtp_user->shop_id
                                ])->lockForUpdate()->first();
                                $new_rtp_i = 0;
                                NewSpin:
                                $new_rtp_i++;
$winTypeTmp = $slotSettings->GetSpinSettings($postData['slotEvent'], $postData['slotBet'] * $postData['slotLines'], $postData['slotLines']);
                            ...
                            if( $scattersRows[1] == 1 && $scattersRows[2] == 1 && $scattersRows[3] == 1 ) 
                            {
                                ...
                                $new_rtp_rtpRange = 50;
                                if($new_rtp_Game->stat_out > 0 && $new_rtp_Game->stat_in > 0) {
                                    $new_rtp_rtpRange = ( $new_rtp_Game->stat_out + $totalWin ) / $new_rtp_Game->stat_in * 100;
                                }
                                //$bet_new_vals = $postData['bet']['lines'];
                                //$bet_new_lines = $postData['bet']['bet'] / 100;
                                $new_rtp_allbets = 2;
                                if(isset($allbet)) {
                                    $new_rtp_allbets = $allbet;
                                } else {
                                    if(array_key_exists('slotBet', $postData) && array_key_exists('slotLines', $postData)) {
                                        $new_rtp_allbets = $postData['slotBet'] * $postData['slotLines'];
                                    } else {
                                        if(array_key_exists('bet', $postData)) {
                                            $new_rtp_allbets = $postData['bet'];
                                        } else {
                                            $new_rtp_allbets = 2;
                                        }
                                    }
                                }
                                if($new_rtp_Game->stat_in > 150 && (floatval($new_rtp_shop->percent) - floatval($new_rtp_rtpRange)) > 3 && $new_rtp_i < 50){
                                    if( (floatval($new_rtp_shop->percent) - floatval($new_rtp_rtpRange)) > 5 ){
                                        if(floatval($totalWin) < floatval($new_rtp_allbets) * 2){
                                            goto NewSpin;
                                        }
                                    } else if ( floatval($totalWin) <= 0){
                                        goto NewSpin;
                                    }
                                } else if($new_rtp_Game->stat_in > 50 && (floatval($new_rtp_rtpRange) - floatval($new_rtp_shop->percent)) > 0 && floatval($totalWin) > 0){
                                    goto NewSpin;
                                }
