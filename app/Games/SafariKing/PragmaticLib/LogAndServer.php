...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        $toServer = [
            'tw='.$toLog['w'],
            'prg_m='.implode(',', $toLog['prg_m']),
            'prg='.implode(',', $toLog['prg']),
        ...
        if($log && array_key_exists('fs', $log)){
            if($log['fs'] == $log['fsmax']){
                ...
                $scatterCnt = 0;
                if(array_key_exists(1, array_count_values($slotArea['SlotArea'])))
                    $scatterCnt = array_count_values($slotArea['SlotArea'])[1];
                $addToLog = [
                    'fsmul' => $log['fsmul'],
                    'fsmax' => $log['fsmax'] + $scatterCnt,
