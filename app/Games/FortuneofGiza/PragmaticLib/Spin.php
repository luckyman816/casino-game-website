...
namespace VanguardLTE\Games\FortuneofGiza\PragmaticLib;
...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        $reelSet = 0;
        $pur1 = $pur;
        if($currentLog && array_key_exists('puri', $currentLog) || $pur1 == '0')    $reelSet = 0;
        ...
        // Check if there are same symbols on one reel
        if(SlotArea::checkReels($slotArea) == 0)
            goto NewSpin;
        // Set the mb values
        ...
        // && !$checkRtp->checkRtp($bet * $lines * 100 / $currentLog['fsmax'], $win['TotalWin'] + $fswin)
        // && $bank->bonus > $bet * $lines * 100 / $currentLog['fsmax']
        // && $newSpinCnt < 4){
        //     $newSpinCnt ++;
        //     goto NewSpin;
        // }
