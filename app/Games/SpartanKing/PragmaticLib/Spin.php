...
namespace VanguardLTE\Games\SpartanKing\PragmaticLib;
...
class Spin
{
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        // set WildBox 2X2
        SlotArea::setWildGroup($slotArea, $currentLog);
       ...
       if (!$win_permission) {
        ...
        && !$checkRtpBonus->checkRtp($bet * $lines * 20 / $currentLog['fsmax'], $win['TotalWin'] + $fswin, $user, $game, $bank)
        && $bank->bonus + $bank->slots > $bet * $lines * 20 / $currentLog['fsmax']
        && $newSpinCnt < 4){
            $newSpinCnt ++;
            goto NewSpin;
        }
