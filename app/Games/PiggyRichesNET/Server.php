<?php 
namespace VanguardLTE\Games\PiggyRichesNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('PiggyRichesNETBonusWin', 0);
                            $slotSettings->SetGameData('PiggyRichesNETFreeGames', 0);
                            $slotSettings->SetGameData('PiggyRichesNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('PiggyRichesNETTotalWin', 0);
                            $slotSettings->SetGameData('PiggyRichesNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('PiggyRichesNETCurrentFreeGame') < $slotSettings->GetGameData('PiggyRichesNETFreeGames') && $slotSettings->GetGameData('PiggyRichesNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('PiggyRichesNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('PiggyRichesNETTotalWin', 0);
                                $slotSettings->SetGameData('PiggyRichesNETBet', $betline);
                                $slotSettings->SetGameData('PiggyRichesNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('PiggyRichesNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('PiggyRichesNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('PiggyRichesNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('PiggyRichesNETCurrentFreeGame', $slotSettings->GetGameData('PiggyRichesNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('PiggyRichesNETBonusWin', $slotSettings->GetGameData('PiggyRichesNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('PiggyRichesNETTotalWin', $slotSettings->GetGameData('PiggyRichesNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('PiggyRichesNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('PiggyRichesNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('PiggyRichesNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('PiggyRichesNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('PiggyRichesNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('PiggyRichesNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('PiggyRichesNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('PiggyRichesNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('PiggyRichesNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('PiggyRichesNETFreeGames');
                                $fsl = $slotSettings->GetGameData('PiggyRichesNETFreeGames') - $slotSettings->GetGameData('PiggyRichesNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('PiggyRichesNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('PiggyRichesNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('PiggyRichesNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('PiggyRichesNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
