<?php 
namespace VanguardLTE\Games\LuckyAnglerNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('LuckyAnglerNETBonusWin', 0);
                            $slotSettings->SetGameData('LuckyAnglerNETFreeGames', 0);
                            $slotSettings->SetGameData('LuckyAnglerNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('LuckyAnglerNETTotalWin', 0);
                            $slotSettings->SetGameData('LuckyAnglerNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('LuckyAnglerNETCurrentFreeGame') < $slotSettings->GetGameData('LuckyAnglerNETFreeGames') && $slotSettings->GetGameData('LuckyAnglerNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('LuckyAnglerNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('LuckyAnglerNETTotalWin', 0);
                                $slotSettings->SetGameData('LuckyAnglerNETBet', $betline);
                                $slotSettings->SetGameData('LuckyAnglerNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('LuckyAnglerNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('LuckyAnglerNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('LuckyAnglerNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('LuckyAnglerNETCurrentFreeGame', $slotSettings->GetGameData('LuckyAnglerNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('LuckyAnglerNETBonusWin', $slotSettings->GetGameData('LuckyAnglerNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('LuckyAnglerNETTotalWin', $slotSettings->GetGameData('LuckyAnglerNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('LuckyAnglerNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('LuckyAnglerNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('LuckyAnglerNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('LuckyAnglerNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('LuckyAnglerNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('LuckyAnglerNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('LuckyAnglerNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('LuckyAnglerNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('LuckyAnglerNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('LuckyAnglerNETFreeGames');
                                $fsl = $slotSettings->GetGameData('LuckyAnglerNETFreeGames') - $slotSettings->GetGameData('LuckyAnglerNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('LuckyAnglerNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('LuckyAnglerNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('LuckyAnglerNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('LuckyAnglerNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
