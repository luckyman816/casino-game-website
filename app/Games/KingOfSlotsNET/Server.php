<?php 
namespace VanguardLTE\Games\KingOfSlotsNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('KingsOfSlotsNETBonusWin', 0);
                            $slotSettings->SetGameData('KingsOfSlotsNETFreeGames', 0);
                            $slotSettings->SetGameData('KingsOfSlotsNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('KingsOfSlotsNETTotalWin', 0);
                            $slotSettings->SetGameData('KingsOfSlotsNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('KingsOfSlotsNETCurrentFreeGame') < $slotSettings->GetGameData('KingsOfSlotsNETFreeGames') && $slotSettings->GetGameData('KingsOfSlotsNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('KingsOfSlotsNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('KingsOfSlotsNETTotalWin', 0);
                                $slotSettings->SetGameData('KingsOfSlotsNETBet', $betline);
                                $slotSettings->SetGameData('KingsOfSlotsNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('KingsOfSlotsNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('KingsOfSlotsNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('KingsOfSlotsNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('KingsOfSlotsNETCurrentFreeGame', $slotSettings->GetGameData('KingsOfSlotsNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('KingsOfSlotsNETBonusWin', $slotSettings->GetGameData('KingsOfSlotsNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('KingsOfSlotsNETTotalWin', $slotSettings->GetGameData('KingsOfSlotsNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('KingsOfSlotsNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('KingsOfSlotsNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('KingsOfSlotsNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('KingsOfSlotsNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('KingsOfSlotsNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('KingsOfSlotsNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('KingsOfSlotsNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('KingsOfSlotsNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('KingsOfSlotsNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('KingsOfSlotsNETFreeGames');
                                $fsl = $slotSettings->GetGameData('KingsOfSlotsNETFreeGames') - $slotSettings->GetGameData('KingsOfSlotsNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('KingsOfSlotsNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('KingsOfSlotsNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('KingsOfSlotsNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('KingsOfSlotsNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
