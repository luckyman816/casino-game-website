...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $doubleChance, $reelSet, $win,
        ...
        $addKeys = ['slm_mv', 'slm_mp', 'slm_lmv', 'slm_lmi', 'trail', 'stf'];
        foreach($addKeys as $val){
            if(array_key_exists($val, $slotArea)){
                $toLog[$val] = $slotArea[$val];
                switch($val){
                    case 'trail':
                        $toServer = array_merge(['trail='.$toLog['trail']], $toServer);
                        break;
                    case 'stf':
                        $stf = $toLog['stf'];
                        $stf = 'w:'.$stf;
                        $toServer = array_merge(['stf='.$stf], $toServer);
                        break;
                    default:
        ...
        // If there is a win
            ...
            if ($log && $log['State'] === 'Respin' || $log['State'] === 'FirstRespin'){
                ...
                $addToServer = [
                    'rs_p='.$addLog['Respin'],
                    'rs_c='.($addLog['Respin'] + 1),
                    'rs_m='.($addLog['Respin'] + 1),
                    'rs=mc',
    ...
    private static function positionsToServer($winLines){
        ...
        foreach ($winLines as $key => $winLine) {
            ...
            $tmb[] = implode(','.$winLine['WinSymbol'].'~', $winLine['Positions']).','.$winLine['WinSymbol'];
            $result[] = $l;
        }
