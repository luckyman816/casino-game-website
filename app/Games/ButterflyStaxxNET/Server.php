<?php 
namespace VanguardLTE\Games\ButterflyStaxxNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('ButterflyStaxxNETBonusWin', 0);
                            $slotSettings->SetGameData('ButterflyStaxxNETFreeGames', 0);
                            $slotSettings->SetGameData('ButterflyStaxxNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('ButterflyStaxxNETTotalWin', 0);
                            $slotSettings->SetGameData('ButterflyStaxxNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('ButterflyStaxxNETCurrentFreeGame') < $slotSettings->GetGameData('ButterflyStaxxNETFreeGames') && $slotSettings->GetGameData('ButterflyStaxxNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('ButterflyStaxxNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('ButterflyStaxxNETTotalWin', 0);
                                $slotSettings->SetGameData('ButterflyStaxxNETBet', $betline);
                                $slotSettings->SetGameData('ButterflyStaxxNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('ButterflyStaxxNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('ButterflyStaxxNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('ButterflyStaxxNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('ButterflyStaxxNETCurrentFreeGame', $slotSettings->GetGameData('ButterflyStaxxNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('ButterflyStaxxNETBonusWin', $slotSettings->GetGameData('ButterflyStaxxNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('ButterflyStaxxNETTotalWin', $slotSettings->GetGameData('ButterflyStaxxNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('ButterflyStaxxNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('ButterflyStaxxNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('ButterflyStaxxNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('ButterflyStaxxNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('ButterflyStaxxNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('ButterflyStaxxNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('ButterflyStaxxNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('ButterflyStaxxNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('ButterflyStaxxNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('ButterflyStaxxNETFreeGames');
                                $fsl = $slotSettings->GetGameData('ButterflyStaxxNETFreeGames') - $slotSettings->GetGameData('ButterflyStaxxNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('ButterflyStaxxNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('ButterflyStaxxNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('ButterflyStaxxNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('ButterflyStaxxNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
