...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        $addKeys = ['trail', 'srf', 'is', 'accm', 'accv', 'acci', 'fsmore'];
        foreach($addKeys as $val){
            if($val == 'fsmore' && !$slotArea[$val])
                continue;
            if(array_key_exists($val, $slotArea)){
                $toLog[$val] = $slotArea[$val];
                switch($val){
                    case 'trail':
                        $toServer[] = 'trail=lvl'.implode('~', $toLog[$val]);
                        break;    
                    case 'srf':
                        $srfs = $toLog[$val];
                        foreach($srfs as $idx => $srf)
                            $srfs[$idx] = implode('~', $srf);
                        $toServer[] = 'srf='.implode(';', $srfs);
                        break;
                    case 'is':
                        $toServer[] = 'is='.implode(',', $toLog[$val]);
                        break;
                    case 'accm':
                        $toServer[]='accm='.implode('~', $toLog['accm']);
                        break;
                    case 'accv':
                        $toServer[] = 'accv='.implode('~', $toLog['accv']);
                        break;
                    default:
