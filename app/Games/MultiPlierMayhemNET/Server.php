<?php 
namespace VanguardLTE\Games\MultiPlierMayhemNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('MultiPlierMayhemNETBonusWin', 0);
                            $slotSettings->SetGameData('MultiPlierMayhemNETFreeGames', 0);
                            $slotSettings->SetGameData('MultiPlierMayhemNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('MultiPlierMayhemNETTotalWin', 0);
                            $slotSettings->SetGameData('MultiPlierMayhemNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('MultiPlierMayhemNETCurrentFreeGame') < $slotSettings->GetGameData('MultiPlierMayhemNETFreeGames') && $slotSettings->GetGameData('MultiPlierMayhemNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('MultiPlierMayhemNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('MultiPlierMayhemNETTotalWin', 0);
                                $slotSettings->SetGameData('MultiPlierMayhemNETBet', $betline);
                                $slotSettings->SetGameData('MultiPlierMayhemNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('MultiPlierMayhemNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('MultiPlierMayhemNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('MultiPlierMayhemNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('MultiPlierMayhemNETCurrentFreeGame', $slotSettings->GetGameData('MultiPlierMayhemNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('MultiPlierMayhemNETBonusWin', $slotSettings->GetGameData('MultiPlierMayhemNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('MultiPlierMayhemNETTotalWin', $slotSettings->GetGameData('MultiPlierMayhemNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('MultiPlierMayhemNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('MultiPlierMayhemNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('MultiPlierMayhemNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('MultiPlierMayhemNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('MultiPlierMayhemNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('MultiPlierMayhemNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('MultiPlierMayhemNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('MultiPlierMayhemNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('MultiPlierMayhemNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('MultiPlierMayhemNETFreeGames');
                                $fsl = $slotSettings->GetGameData('MultiPlierMayhemNETFreeGames') - $slotSettings->GetGameData('MultiPlierMayhemNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('MultiPlierMayhemNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('MultiPlierMayhemNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('MultiPlierMayhemNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('MultiPlierMayhemNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
