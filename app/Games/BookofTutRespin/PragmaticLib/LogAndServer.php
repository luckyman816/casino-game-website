...
class LogAndServer
{
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, $win, $pur, 
        ...
        $addKeys = ['trail', 'stf', 'is', 'rs_p', 'rs_c', 'rs_m', 'rs_t', 'rswin', 'g'];
        ...
        if($pur === '0'){
            ...
            $addToLog = [
                ...
                'puri' => 0,
                'bw' => 1,
                'g' => '{buy:{ask:"0",bgid:"0",bgt:"69",ch_k:"bf_h1,bf_h2,bf_m1,bf_m2,bf_a,bf_k,bf_q,bf_j,bf_t",ch_v:"0,1,2,3,4,5,6,7,8",end:"0",rw:"0.00"}}'
            ];
            $addToServer = [
                'purtr=1',
                'puri=0',
        ...
        if($log && array_key_exists('fs', $log)){
            ...
            if(!array_key_exists('trail', $toLog)){
                $toLog['trail'] = $log['trail'];
                $toServer[] = 'trail=sp_s~'.$toLog['trail'];
            }
            $toLog = array_merge($toLog, $addToLog);
            $toServer = array_merge($toServer, $addToServer);
        }
