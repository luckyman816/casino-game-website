...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            ...
            if ($command == 'commands') {
                ...
                if (is_array($gameData) && array_key_exists('gameActionId', $gameData) && $gameData['gameActionId'] == 'INIT') {
                    if (!$log) $log = ["Reels" => $settings->randomSlotArea, 'Winnings' => [], 'TotalWin' => 0, 'Bet' => 10, 'Lines' => 10, 'WildParameters' => []];
                ...
                switch ($command) {
                    case 'PLAY':
                        ...
                        $lines = count($com->wagers);
                        // если не хватает денег
                        if ((double)($bet / 100) > $user->balance) {
                            if (!$log) $log = ["SlotArea" => $settings->randomSlotArea, 'Winnings' => [], 'Bet' => 0, 'TotalWin' => 0, 'WildParameters' => []];
                            return response()->json(Init::get((int)round($user->balance * 100), $log, true));
                        }

                        list($winnings, $readyReels, $slotArea, $totalWin, $wildParameters) = Spin::get($bet, $lines, $settings, $bank, $shop->percent, $game->stat_in, $game->stat_out);
                        ...
                        $toLog = ['Winnings' => $winnings, 'Reels' => $readyReels, 'SlotArea' => $slotArea, 'TotalWin' => $totalWin, 'Bet' => $bet, 'Lines' => $lines, 'WildParameters' => $wildParameters];

                        // вернуть ответ с сервера
                        $response = Play::get($wildParameters, (int)round($user->balance * 100), $winnings, $readyReels, $bet, $totalWin);
