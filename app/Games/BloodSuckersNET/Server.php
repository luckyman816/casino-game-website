<?php 
namespace VanguardLTE\Games\BloodSuckersNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('BloodSuckersNETBonusWin', 0);
                            $slotSettings->SetGameData('BloodSuckersNETFreeGames', 0);
                            $slotSettings->SetGameData('BloodSuckersNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('BloodSuckersNETTotalWin', 0);
                            $slotSettings->SetGameData('BloodSuckersNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('BloodSuckersNETCurrentFreeGame') < $slotSettings->GetGameData('BloodSuckersNETFreeGames') && $slotSettings->GetGameData('BloodSuckersNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('BloodSuckersNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('BloodSuckersNETTotalWin', 0);
                                $slotSettings->SetGameData('BloodSuckersNETBet', $betline);
                                $slotSettings->SetGameData('BloodSuckersNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('BloodSuckersNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('BloodSuckersNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('BloodSuckersNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('BloodSuckersNETCurrentFreeGame', $slotSettings->GetGameData('BloodSuckersNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('BloodSuckersNETBonusWin', $slotSettings->GetGameData('BloodSuckersNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('BloodSuckersNETTotalWin', $slotSettings->GetGameData('BloodSuckersNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('BloodSuckersNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('BloodSuckersNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('BloodSuckersNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('BloodSuckersNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('BloodSuckersNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('BloodSuckersNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('BloodSuckersNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('BloodSuckersNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('BloodSuckersNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('BloodSuckersNETFreeGames');
                                $fsl = $slotSettings->GetGameData('BloodSuckersNETFreeGames') - $slotSettings->GetGameData('BloodSuckersNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('BloodSuckersNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('BloodSuckersNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('BloodSuckersNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('BloodSuckersNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
