    ...
    public static function getResult($slotArea, $index, $counter, $bet, $lines, $reelSet, &$win, $pur, 
        ...
        if($log && array_key_exists('fs', $log)){
            ...
            if($log && array_key_exists('accv', $log)){
                ...
                $accv[1][0] = $accv[1][2];
                $minus = rand(200, 400);
                $accv[1][1] = $accv[1][0] - $accv[1][0] * $minus / 1000;
                $accv[1][1] = number_format($accv[1][1], 0, '.', '');
                $accv[1][1] = $accv[1][1] < 0 ? 0 : $accv[1][1];
                $accv[1][2] = $accv[1][1];
            }
            else {
                $accv[] = [0, 4, 0, 0];
            ...
            if(count(array_keys($slotArea['SlotArea'], 2))){
                $accv[0][0] += 1;
                $accv[0][0] = $accv[0][0] > 12 ? 12 : $accv[0][0];
                $accv[0][3] = 1;
            }
            if(!count(array_keys($slotArea['SlotArea'], 2)))
                foreach($slotArea['mo'] as $mo)
                    $accv[1][2] += $mo;
            if(array_key_exists('mo_tv', $win)){
                $win['mo_tv'] += $accv[1][2] * $win['gold'] * $win['multi'];
                $win['TotalWin'] += $accv[1][2] * $bet * $win['gold'] * $win['multi'];
            }

            var_dump('!!!2');
            if($log['fs'] == $log['fsmax'] && $accv[0][0] < $accv[0][1] * $accv[0][2] + 4 || $log['fs'] == $log['fsmax'] && $accv[0][2] > 2){
                $addToLog = [
                    'fs_total' => $log['fs'],
            ...
            // }
            $addToLog['accm'] = 'cp~tp~lvl~sc;ns~nr~ne';
            $addToLog['acci'] = '0;1';
            if(count(array_keys($slotArea['SlotArea'], 2))){
                $addToLog['mo_wpos'] = $slotArea['mo_wpos'];
                $addToServer[] = 'mo_wpos='.implode(',', $addToLog['mo_wpos']);
            }
            $addToLog['accv'] = $accv;
            $addToServer[] = 'accm='.$addToLog['accm'];
            $addToServer[] = 'acci='.$addToLog['acci'];
            $addToServer[] = 'accv='.implode(';', [implode('~', $accv[0]), implode('~', $accv[1])]);
            if(array_key_exists('mo_tv', $win)){
            ...
            if(array_key_exists('trail', $slotArea)){
                $addToLog['trail'] = $slotArea['trail'];
                $addToServer[] = 'trail='.$addToLog['trail'];
                $addToLog['stf'] = $slotArea['stf'];
                $addToServer[] = 'stf='.$addToLog['stf'];
                $addToLog['is'] = $slotArea['is'];
                $addToServer[] = 'is='.implode(',', $addToLog['is']);
            }
            $toLog = array_merge($toLog, $addToLog);
            $toServer = array_merge($toServer, $addToServer);
        }
