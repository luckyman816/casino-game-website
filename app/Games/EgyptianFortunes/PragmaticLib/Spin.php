...
namespace VanguardLTE\Games\EgyptianFortunes\PragmaticLib;
    ...
    public static function spinResult($user, $game, $bet, $lines, $log, $gameSettings, $index, $counter, $callbackUrl, $pur, $bank, $shop, $jpgs){
        ...
        if ($currentLog &&
            (array_key_exists('state', $currentLog) && $currentLog['state'] != 'spin' && $currentLog['state'] != 'lastRespin' ||
                ...
                || $currentLog && array_key_exists('rs_c', $currentLog)){
            $changeBalance = 0;
        }else{
        ...
        $reelSet = 0;
        $pur1 = $pur;
        if ($currentLog && array_key_exists('fs', $currentLog)) $reelSet = $currentLog['reel_set']; // if free spins - then the set of reels is 4th
        ...
        if ($pur1 === '0') 
            ...
            if($currentLog && array_key_exists('fs', $currentLog) && $currentLog['fs'] == $currentLog['fsmax'])
                ...
                $max = 5;
                if($currentLog && array_key_exists('rs_c', $currentLog))
                    $max = 2;
                $slotArea['ScatterCount'] = BuyFreeSpins::getFreeSpin($slotArea['SlotArea'], $gameSettings, 0, $max);
            }
        }
        var_dump('2');
        ...
        // handle energy spin
        SlotArea::handleRS($gameSettings, $slotArea, $currentLog, $win);
        var_dump('3');
