<?php 
namespace VanguardLTE\Games\DeadOrAliveNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('DeadOrAliveNETBonusWin', 0);
                            $slotSettings->SetGameData('DeadOrAliveNETFreeGames', 0);
                            $slotSettings->SetGameData('DeadOrAliveNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('DeadOrAliveNETTotalWin', 0);
                            $slotSettings->SetGameData('DeadOrAliveNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('DeadOrAliveNETCurrentFreeGame') < $slotSettings->GetGameData('DeadOrAliveNETFreeGames') && $slotSettings->GetGameData('DeadOrAliveNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('DeadOrAliveNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('DeadOrAliveNETTotalWin', 0);
                                $slotSettings->SetGameData('DeadOrAliveNETBet', $betline);
                                $slotSettings->SetGameData('DeadOrAliveNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('DeadOrAliveNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('DeadOrAliveNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('DeadOrAliveNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('DeadOrAliveNETCurrentFreeGame', $slotSettings->GetGameData('DeadOrAliveNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('DeadOrAliveNETBonusWin', $slotSettings->GetGameData('DeadOrAliveNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('DeadOrAliveNETTotalWin', $slotSettings->GetGameData('DeadOrAliveNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('DeadOrAliveNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('DeadOrAliveNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('DeadOrAliveNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('DeadOrAliveNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('DeadOrAliveNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('DeadOrAliveNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('DeadOrAliveNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('DeadOrAliveNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('DeadOrAliveNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('DeadOrAliveNETFreeGames');
                                $fsl = $slotSettings->GetGameData('DeadOrAliveNETFreeGames') - $slotSettings->GetGameData('DeadOrAliveNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('DeadOrAliveNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('DeadOrAliveNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('DeadOrAliveNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('DeadOrAliveNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
