<?php 
namespace VanguardLTE\Games\JackHammer2NET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('JacksHammerNETBonusWin', 0);
                            $slotSettings->SetGameData('JacksHammerNETFreeGames', 0);
                            $slotSettings->SetGameData('JacksHammerNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('JacksHammerNETTotalWin', 0);
                            $slotSettings->SetGameData('JacksHammerNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('JacksHammerNETCurrentFreeGame') < $slotSettings->GetGameData('JacksHammerNETFreeGames') && $slotSettings->GetGameData('JacksHammerNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('JacksHammerNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('JacksHammerNETTotalWin', 0);
                                $slotSettings->SetGameData('JacksHammerNETBet', $betline);
                                $slotSettings->SetGameData('JacksHammerNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('JacksHammerNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('JacksHammerNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('JacksHammerNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('JacksHammerNETCurrentFreeGame', $slotSettings->GetGameData('JacksHammerNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('JacksHammerNETBonusWin', $slotSettings->GetGameData('JacksHammerNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('JacksHammerNETTotalWin', $slotSettings->GetGameData('JacksHammerNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('JacksHammerNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('JacksHammerNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('JacksHammerNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('JacksHammerNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('JacksHammerNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('JacksHammerNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('JacksHammerNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('JacksHammerNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('JacksHammerNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('JacksHammerNETFreeGames');
                                $fsl = $slotSettings->GetGameData('JacksHammerNETFreeGames') - $slotSettings->GetGameData('JacksHammerNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('JacksHammerNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('JacksHammerNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('JacksHammerNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('JacksHammerNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
