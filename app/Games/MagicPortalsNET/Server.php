<?php 
namespace VanguardLTE\Games\MagicPortalsNET
{
    set_time_limit(5);
    class Server
    {
                ...
                try
                {
                    ...
                    switch( $aid ) 
                    {
                        case 'init':
                            ...
                            $slotSettings->SetGameData('MagicPortalsNETBonusWin', 0);
                            $slotSettings->SetGameData('MagicPortalsNETFreeGames', 0);
                            $slotSettings->SetGameData('MagicPortalsNETCurrentFreeGame', 0);
                            $slotSettings->SetGameData('MagicPortalsNETTotalWin', 0);
                            $slotSettings->SetGameData('MagicPortalsNETFreeBalance', 0);
                            $freeState = '';
                            if( $lastEvent != 'NULL' ) 
                            {
                            ...
                            if( $slotSettings->GetGameData('MagicPortalsNETCurrentFreeGame') < $slotSettings->GetGameData('MagicPortalsNETFreeGames') && $slotSettings->GetGameData('MagicPortalsNETFreeGames') > 0 ) 
                            {
                        ...
                        case 'spin':
                            ...
                            if( $postData['slotEvent'] != 'freespin' ) 
                            {
                                ...
                                $slotSettings->SetGameData('MagicPortalsNETCurrentFreeGame', 0);
                                $slotSettings->SetGameData('MagicPortalsNETTotalWin', 0);
                                $slotSettings->SetGameData('MagicPortalsNETBet', $betline);
                                $slotSettings->SetGameData('MagicPortalsNETDenom', $postData['bet_denomination']);
                                $slotSettings->SetGameData('MagicPortalsNETFreeBalance', sprintf('%01.2f', $slotSettings->GetBalance()) * 100);
                                $bonusMpl = 1;
                            }
                            else
                            {
                                $postData['bet_denomination'] = $slotSettings->GetGameData('MagicPortalsNETDenom');
                                $slotSettings->CurrentDenom = $postData['bet_denomination'];
                                $slotSettings->CurrentDenomination = $postData['bet_denomination'];
                                $betline = $slotSettings->GetGameData('MagicPortalsNETBet');
                                $allbet = $betline * $lines;
                                $slotSettings->SetGameData('MagicPortalsNETCurrentFreeGame', $slotSettings->GetGameData('MagicPortalsNETCurrentFreeGame') + 1);
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $slotSettings->SetGameData('MagicPortalsNETBonusWin', $slotSettings->GetGameData('MagicPortalsNETBonusWin') + $totalWin);
                                $slotSettings->SetGameData('MagicPortalsNETTotalWin', $slotSettings->GetGameData('MagicPortalsNETTotalWin') + $totalWin);
                            }
                            else
                            {
                                $slotSettings->SetGameData('MagicPortalsNETTotalWin', $totalWin);
                            }
                            $fs = 0;
                            if( $scattersCount >= 4 ) 
                            {
                                $slotSettings->SetGameData('MagicPortalsNETFreeStartWin', $totalWin);
                                $slotSettings->SetGameData('MagicPortalsNETBonusWin', $totalWin);
                                $slotSettings->SetGameData('MagicPortalsNETFreeGames', $slotSettings->slotFreeCount[$scattersCount]);
                                $fs = $slotSettings->GetGameData('MagicPortalsNETFreeGames');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=freespin&freespins.left=' . $fs . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=basic%2Cfreespin&freespins.totalwin.coins=0&freespins.total=' . $fs . '&freespins.win.cents=0&gamestate.current=freespin&freespins.initial=' . $fs . '&freespins.win.coins=0&freespins.betlevel=' . $slotSettings->GetGameData('MagicPortalsNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            ...
                            $winstring = '';
                            $slotSettings->SetGameData('MagicPortalsNETGambleStep', 5);
                            $hist = $slotSettings->GetGameData('MagicPortalsNETCards');
                            $isJack = 'false';
                            if( $totalWin > 0 ) 
                            {
                                $state = 'gamble';
                            ...
                            if( $postData['slotEvent'] == 'freespin' ) 
                            {
                                $totalWin = $slotSettings->GetGameData('MagicPortalsNETBonusWin');
                                ...
                                $fs = $slotSettings->GetGameData('MagicPortalsNETFreeGames');
                                $fsl = $slotSettings->GetGameData('MagicPortalsNETFreeGames') - $slotSettings->GetGameData('MagicPortalsNETCurrentFreeGame');
                                $freeState = '&freespins.betlines=0%2C1%2C2%2C3%2C4%2C5%2C6%2C7%2C8%2C9%2C10%2C11%2C12%2C13%2C14%2C15%2C16%2C17%2C18%2C19&freespins.totalwin.cents=0&nextaction=' . $nextaction . '&freespins.left=' . $fsl . '&freespins.wavecount=1&freespins.multiplier=1&gamestate.stack=' . $stack . '&freespins.totalwin.coins=' . $totalWin . '&freespins.total=' . $fs . '&freespins.win.cents=' . ($totalWin / $slotSettings->CurrentDenomination * 100) . '&gamestate.current=' . $gamestate . '&freespins.initial=' . $fs . '&freespins.win.coins=' . $totalWin . '&freespins.betlevel=' . $slotSettings->GetGameData('MagicPortalsNETBet') . '&totalwin.coins=' . $totalWin . '&credit=' . $balanceInCents . '&totalwin.cents=' . ($totalWin * $slotSettings->CurrentDenomination * 100) . '&game.win.amount=' . ($totalWin / $slotSettings->CurrentDenomination) . '';
                                $curReels .= $freeState;
                            }
                            $response = '{"responseEvent":"spin","responseType":"' . $postData['slotEvent'] . '","serverResponse":{"freeState":"' . $freeState . '","slotLines":' . $lines . ',"slotBet":' . $betline . ',"totalFreeGames":' . $slotSettings->GetGameData('MagicPortalsNETFreeGames') . ',"currentFreeGames":' . $slotSettings->GetGameData('MagicPortalsNETCurrentFreeGame') . ',"Balance":' . $balanceInCents . ',"afterBalance":' . $balanceInCents . ',"bonusWin":' . $slotSettings->GetGameData('MagicPortalsNETBonusWin') . ',"totalWin":' . $totalWin . ',"winLines":[],"Jackpots":' . $jsJack . ',"reelsSymbols":' . $jsSpin . '}}';
