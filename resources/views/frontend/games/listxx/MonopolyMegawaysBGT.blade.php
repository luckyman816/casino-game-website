...
<head>
    ...
    <script type="text/javascript">
        ...
        if (document.location.href.split("?")[1] == undefined) {
            document.location.href = document.location.href +
                '/?envid=eur&stage=1&gameid=monopolymegaways96&operatorid=241&sessionid=&currency=&lang=en_us&mode=demo&secure=true&type=nextgen&clock=true&device=mobile&lobbyurl=&depositurl=&ogsgameid=70476&nyxroot=&&lang=en&w=&lang=en';
        } else if (document.location.href.split("?api_exit")[1] != undefined) {

            document.location.href = uparts[0] +
                '/?envid=eur&stage=1&gameid=monopolymegaways96&operatorid=241&sessionid=&currency=&lang=en_us&mode=demo&secure=true&type=nextgen&clock=true&device=mobile&lobbyurl=&depositurl=&ogsgameid=70476&nyxroot=&&lang=en&w=&lang=en' +
                uparts[1];
        }
    ...
    <script type="text/javascript">
        ...
        function initilise() {
            ...
            urlParameters.inputLangCode = ogsParams.lang ? ogsParams.lang : getUrlParam(
                'language'); // get the language code passed in
            ...
            document.getElementById("icon").setAttribute("href", urlParameters.gameFolderName +
            ...
            urlParameters.realitycheck_uk_autospin = ogsParams.realitycheck_uk_autospin ? ogsParams
                .realitycheck_uk_autospin :
                getUrlParam("realitycheck_uk_autospin");
            urlParameters.realitycheck_uk_timestamp = ogsParams.realitycheck_uk_timestamp ? ogsParams
                .realitycheck_uk_timestamp :
        ...
        function getGutterSize() {
            for (var a in gConfigObj.iframeGutters) {
                ...
                if (obj.clientType.length > 0 && ~obj.clientType.indexOf(urlParameters.clientType) || obj.clientType
                    .length == 0) {} else {
                    continue;
                }
                if (obj.operatorId.length > 0 && ~obj.operatorId.indexOf(opid) || obj.operatorId.length == 0) {} else {
                    continue;
                }
                if (obj.jurisdiction.length > 0 && ~obj.jurisdiction.indexOf(urlParameters.jurisdiction) || obj.jurisdiction
                    .length == 0) {} else {
                    continue;
                }
                if (obj.gameName.length > 0 && ~obj.gameName.indexOf(urlParameters.gameName) || obj.gameName.length ==
                    0) {} else {
                    continue;
                }
        ...
        function updateBalance() {
            return $.ajax({
                ...
                if (wrapperinterface) {
                    ...
                    gameiframe.contentWindow.apiExt("SET_BALANCE",
                        balanceAmount); // Call the game to update the balance
                }
                ...
                if (isNJ) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            var payload = xmlGDMResponse.PAYLOAD.replace("<![CDATA[", "").replace("]]>",
                                ""); //.replace(/"/g, "");
            ...
            if (isNJ) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        var oldMsg = lastXmlResponse != null && lastResponse != null && lastResponse.MESSAGE ?
                        ...
                        if (~payload.indexOf("MSGID=INIT")) {
                            if (currencyFormat != "") {
                                payload = payload.replace(payload.substring(payload.indexOf("CUR"), payload
                                    .indexOf(
                    ...
                    error: function(jqXHR, textStatus, errorThrown) {
                        ...
                        gtag('event', 'sendMsgToServer', {
                            'operatorID': getOperatorID(),
                            'operatorName': getOperatorName(),
                            'state': 'error',
                            'textStatus': textStatus,
                            'status': String(jqXHR.status),
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (payload.indexOf("MSGID=FREE_GAME") == -1 && payload.indexOf("MSGID=FEATURE") == -
                            1 && payload.indexOf("B=") > -1 && payload.indexOf("AB=") > -1) {
                        ...
                        if (xmlGDMString.indexOf("MSGID=ERROR") > -1 && ~xmlGDMString.indexOf(
                                "<REALITYCHECK")) {
        ...
        function getLanguageCode() {
            ...
            if (xmlhttp.status >= 400) { // using 5 character codes
                ...
                xmlhttp.open('HEAD', urlParameters.gameFolderName + "/language/" + translateLangCode(urlParameters
                        .inputLangCode) +
                    "/language.js", false);
                xmlhttp.send();
        ...
        function getCustomSettings() {
            ...
            return {
                ...
                "gambleCount": lookupOperatorConfig(opid, gConfigObj.inclusiveOperators).gambleCount != undefined ?
                    lookupOperatorConfig(opid, gConfigObj.inclusiveOperators).gambleCount : getGameDefaultGambleCount(
                        urlParameters.gameName)
            };
        }
            ...
            try {
                if (lastResponse.MESSAGE) {
                    try {
                        ...
                        if (typeof(lastResponse.MESSAGE.TYPE) !== "undefined") {
                            if (~lastResponse.MESSAGE.TYPE.toLowerCase().indexOf("realitycheck")) {
                                ...
                                if (typeof(lastResponse.MESSAGE.TEXT) === "undefined") {
                                    ...
                                    if (wrapperLang == "en_us" || wrapperLang == "en") {
                                        if (sessionTime > 3600) {
                                            ...
                                            if (minutes == 0) {
                                                text = "You have been playing for " + hours +
                                                    " hour(s). Press CONTINUE to keep playing";
                                            } else {
                                                text = "You have been playing for " + hours + " hour(s) and " + minutes +
                                                    " minute(s). Press CONTINUE to keep playing";
                                            }
                                            ...
                                            if (seconds == 0) {
                                                text = "You have been playing for " + minutes +
                                                    " minute(s). Press CONTINUE to keep playing";
                                            } else {
                                                text = "You have been playing for " + minutes + " minute(s) and " +
                                                    seconds + " second(s). Press CONTINUE to keep playing";
                                            }

                                        } else {
                                            text = "You have been playing for " + sessionTime +
                                                " second(s). Press CONTINUE to keep playing";
                                        }
                                    } else {
                                        if ((sessionTime % 60) == 0) {
                                            text = wrappertexts.language[lang].reality_check_text.replace("%1", (
                                                sessionTime / 60).toFixed(0));
                                        } else {
                                            text = wrappertexts.language[lang].reality_check_text.replace("%1", (
                                                sessionTime / 60).toFixed(1));
                                        }
                                    }
                                ...
                                if (optionElements.length > 2) {
                                    if (realityCheck.RC_NOTIFICATION_LINK[2] != "" && optionElements[2].getAttribute(
                                            "action") == "") {
                                        lastXmlResponse.getElementsByTagName('OPTION')[2].setAttribute("action",
                                            realityCheck.RC_NOTIFICATION_LINK[2]);
                                    }
                                }
                            }
                        }
                        ...
                        console.warn("error trying to call handleMessageTrigger: ", "message parameter ", lastXmlResponse,
                            e);
                    }
                }
            } catch (err) {}
                ...
                if (iframeMode != "true") {
                    ...
                    if (typeof wrapperinterface != 'undefined' && wrapperinterface) {
                        ...
                        if (isGameHalfScaled(urlParameters.gameName)) {
                            gameAPI("RESIZE", $(window).width() * 2, ($(window).height() - iframeHeightOffset) *
                                2); // TODO: Get the correct size
                        } else {
                            gameAPI("RESIZE", $(window).width(), $(window).height() -
                                iframeHeightOffset); // TODO: Get the correct size
                        }
                        //}
                    }
                } else {
